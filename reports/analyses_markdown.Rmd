...
# Color fruits
```{r }
data_color <- read.table("../data/Data_color_fruits.csv", header=T, sep=",", dec=".")
head(data_color)
...
P <- read.table("releve_donnees_phosphore.csv", header=T, sep=",", dec = "," )
Fr <- read.table("releve_donnees_fruits.csv", header=T, sep=",", dec = "." )
...
```{r }
ggplot(P, aes(x=concentration, y=nb_oeufs, group=concentration))+geom_boxplot(notch=F,aes(fill=concentration))+theme_bw()+ggtitle("Nombre de pontes en fonction de la concentration en phosphore du milieu")
